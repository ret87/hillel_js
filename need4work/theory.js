...
// Mетод toUpperCase(), который возвращает строку в верхнем регистре:
...
// alert( 12.345.toFixed(1) ); // 12.3

    // Infinity
// Infinity – особенное численное значение, которое ведет себя в точности как математическая бесконечность ∞ (бесконечно большое число)
// Бесконечность можно присвоить и в явном виде: var x = Infinity.
// Infinity больше любого числа.
// Добавление к бесконечности не меняет её.

    // NaN
// математическая операция которая не может быть совершена - возвращается специальное значение NaN (Not-A-Number).
// Значение NaN – единственное в своем роде, которое не равно ничему, включая себя.
// - Значение NaN можно проверить специальной функцией isNaN(n), она преобразует аргумент к числу и возвращает true, 
// если NaN, и false – для любого другого значения.
...
// Самое простое преобразование:  +             (другие операторы так же работают)
// Если строка не является в точности числом, то результат будет NaN (исключение пробельные символы)
...
// alert( parseInt('12px') ) // 12, ошибка на символе 'p'
    ...
    // // Проверка на число
    // * isNaN(numb) - выдаст true если не число и false если число-можно проверить через простой if(isNaN(num){не число} else {число});
...
// var n = 10; console.log(n.toString(2));  => 0101

    // Округление
// Math.floor - округляет вниз                      console.log( Math.floor(3.1) );  // 3
// Math.ceil - округляет вверх                      console.log( Math.ceil(3.1) );   // 4
// Math.round - округляет до ближайшего целого      console.log( Math.round(3.1) );  // 3
    // Короткие записик округления
// ~~number     - скоращенная запись округления до целого число вниз            console.log( ~~3.9 );  // 3
// number ^ 0   - скоращенная запись округления до целого число вниз         console.log( 3.9 + 4.9 ^ 0 );  // 8
...
// num.toFixed(precision)   - округляет (как Math.round) число num до точности precision и возвращает результат в виде строки

    // Неточные вычисления
// Hа число выделяется ровно 8 байт(=64 бита), не больше и не меньше. По этому до определённых чисел существует небольшая неточность.
// Что бы получать точные вычисления числел:
// * (numb1 * 10 + numb3 * 10) / 10 );  -Сделать целыми умножив на 10 с 0 после запятой, сложить числа, а потом поделить на тоже число.
// * +result.toFixed(10) -  Сложить, а затем округлить до разумного знака после запятой. до 10-го знака обычно бывает достаточно.
...
// Все классы назваются с Большой буквы: ParrentTwo.prototype = new ParrentOne();
...
// * arg1, arg2… - Аргументы, которые нужно передать функции. Исполнение функции произойдёт спустя время, указанное в параметре delay.
...
// * Bремя работы функции никак не учитывается, оно «съедает» часть интервала.
// * Если функция и выполняется дольше, чем пауза setInterval, то вызовы будут происходить вообще без перерыва.
// var timerId = setInterval(func / code, delay[, arg1, arg2...])

    // Рекурсивный setTimeout
// * По сути используется стандартное замыкание, вызова параметра функции - внутри функции с именем параметра
...
// * При рекурсивном setTimeout задержка всегда фиксирована и равна delay.

// Cложные задачи разбиваются на части, выполнение каждой части запускается через мини-интервал после предыдущей - даёт браузеру время.
...
//  Что бы явно указать this потребуется помощь методов call и apply

    // Метод call
// func.call(context, arg1, arg2, ...) - синтаксис
// context - это объект или функция, из которой брать аргументы или свойства, методы
// Приравнивая аргументы как значения зависимые от this - в зависимости кем и что будет в аргументах call - будут разные свойства
...
// После контекста в call можно передать аргументы для функции. И конструировать ответ из указанных свойств объекта.
...
// };
// printArgs(1, 2, 3);
// В строке (1) объявлен пустой массив [] и скопирован его метод [].join. Обратим внимание, мы не вызываем его, а просто копируем. 
// Функция, в том числе встроенная – обычное значение, мы можем скопировать любое свойство любого объекта, и [].join здесь не исключение
...
// func.call(context, arg1, arg2, ...)
    ...
    // Вызов одного метода прототипа из другого прототипа - можно использовав его как параметр в функции
