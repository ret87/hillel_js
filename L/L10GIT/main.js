...
// $ git branch -D name   - удалит ветку даже если вы не слили с неё наработки 
        ...
        // Ветвление в Git - Приёмы работы с ветками

// - Когда вы выполняете ветвление и слияние, всё происходит только в вашем репозитории — связь с сервером не осуществляется.

    // Долгоживущие ветки
// - Ветку мастер стараются делать стабильной и сливать в неё только проверенные ветки.
// - Ветку разработка тоже стараются делать стабильной, переодически подвергая её тестам.
// - Остальные по нуждам проектов.
    
    // Тематические ветки
// - Тематическая ветка — недолговечная ветка, которую вы создаёте и используете 
// для работы над некоторой отдельной функциональностью или для вспомогательной работы. 
// - Когда все изменения разбиты по веткам и определённым темам, намного проще понять, 
// что было сделано, во время проверки и просмотра кода. 
...
// - Oни двигаются автоматически всякий раз, когда вы осуществляете связь по сети. 
// - Oни действуют как закладки для напоминания о том, где ветки в удалённых реп. находились во время последнего подключения к ним.
// - (имя удал. репоз.)/(ветка)   -   так выглядят удалённые ветки
// - origin/master    - пример
...
// $ git fetch origin   - синхронизирует удалённые ветки репозитория с вашими
    ...
    // Отправка изменений
// - Нужно явно отправить те ветки, которыми вы хотите поделиться
// - Можно отправлять только те тематические ветки, над которыми вы хотите работать с кем-то совместно.
// - Когда при получении данных у вас появляются новые удалённые ветки, вы не получаете автоматически для них локальных копий. 
// $ git merge имяУдСер/НоваяУдВетка    - так можно слить себе ветку с сервера, которой нет у нас на локальном репозитории и отслеж её
...
// $ git checkout -b sf origin/serverfix    - пример

    // Удаление веток на удалённом сервере
// git push [удал. сервер] :[ветка]     - удалить ветку на удалённом сервере
// "возьми ничто в моём репозитории и сделай так, чтобы в [удал. ветка] было то же самое"
...
// При перемещении, отменяются существующие коммиты и создаются новые, которые похожи на старые, но являются другими.
// Если перемещать коммиты, которые уже были представлены для общего доступа, и люди, возможно, основывали свою работу на них, 
// тогда можно получить разные неприятные проблемы.
